...
public class AvroSort extends Configured implements Tool {
	...
	static class SortMapper<K> extends AvroMapper<K, Pair<K, K>> {
		/**
		 * recordSchema --> map() --> (recordSchema,recordSchema)
		 */
	...
	static class SortReducer<K> extends AvroReducer<K, K, K> {
		/**
		 * (recordSchema,[recordSchema1,...]) --> reduce() -->recordSchema
		 */
	...
	public int run(String[] args) throws Exception {
		 ...
		 * (recordSchema,[recordSchema1,...]) --> reduce() -->recordSchema
		 */
		Schema recordSchema = new Schema.Parser().parse(new File(schemaFile));
		AvroJob.setInputSchema(conf, recordSchema);
		Schema intermediateSchema = Pair.getPairSchema(recordSchema, recordSchema);
		AvroJob.setMapOutputSchema(conf, intermediateSchema);
		AvroJob.setOutputSchema(conf, recordSchema);
